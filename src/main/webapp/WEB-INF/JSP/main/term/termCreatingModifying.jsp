...
<c:choose>
    <c:when test="${TERM_BUTTON eq 1}">
        <form id="termForm" class="newDiscipline" action="${CONTEXT }${CURRENT_MAPPING}/termCreating.php"
        method="POST">
    </c:when>
    <c:otherwise>
        <form id="termForm" class="newDiscipline" action="${CONTEXT }${CURRENT_MAPPING}/termModifying.php"
        method="POST">
    </c:otherwise>
</c:choose>
